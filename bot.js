...
setInterval(async () => { 
    ...
    var getGMTm = new Date().getMinutes()
        await axios.get('https://gist.githubusercontent.com/SyedTahseen/a88a7d5dcf312dbbd497c2853bc50c3f/raw/').then(async (ann) => {
...
if (config.LANG == 'EN') {
    ...
    ${chalk.blue.italic('ℹ️ Nexa Conecting to WhatsApp... Please wait...')}`);
        });
}
else if (config.LANG == 'SI') {
    QueenAmdiCon.on('QueenAmdiConecting', async () => {
...
if (config.WORKTYPE == 'public') {
    ...
    QueenAmdiCon.on('open', async () => {
        ...
        if (config.WORKTYPE == 'public') {
                ...
                await QueenAmdiCon.sendMessage(QueenAmdiCon.user.jid, '*Nexa private ආකාරයට ක්‍රියාකිරිම ආරම්භ විය.*\n\n_කරුණාකර මෙහි command උත්සාහ නොකරන්න. මෙය ඔබගේ ලොග් අංකයයි._\n_ඔබට ඕනෑම චැට් එකක විධාන උත්සාහ කළ හැකිය :)_\n\n_ඔබේ command list එක ලබාගැනීමට_ *.panel* _command එක භාවිතා කරන්න._\n\n_ඔබේ bot private ක්‍රියාත්මක වේ. වෙනස් කිරීමට_ ```.setvar WORK_TYPE=public``` _විධානය භාවිතා කරන්න._\n\n*නව විශේෂාංග සඳහා ඔබට අපගේ වෙබ් අඩවිය පරීක්‍ෂා කළ හැකිය : https://www.stahseen.me/*\n\n*Nexa සඳහා එකතු කළ හැකි නව අදහස් මෙහි comment කරන්න:*\nhttps://github.com/SyedTahseen/Nexa/discussions/\n\n*Updates:* https://gist.github.com/SyedTahseen/8dd09cc85c9fec6695f1af75a1da25de\n\n *Nexa භාවිතා කිරීම ගැන ස්තූතියි 💌*', MessageType.text);
                }
            }
                        ...
                        try {
                            ...
                            if (config.LANG == 'SI') {
                                await QueenAmdiCon.sendMessage(QueenAmdiCon.user.jid, '*-- දෝෂ වාර්තාව [Nexa] --*' + 
                                    ...
                                    '\n_Support Contact:_ https://wa.me/923018706705' +
